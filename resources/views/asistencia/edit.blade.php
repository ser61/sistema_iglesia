...
@section('content')
    ...
    <div class="panel panel-primary">
        <div class="panel-heading" align="center" style="font-size: 20px">
            <strong>Formulario Para Edita Asistencia</strong>
        </div>
        <div class="panel-body">
            {!!Form::model($asistente,['route'=> ['asistencia.update',$asistente->id],'method'=>'PUT'])!!}
            <br><br>
            ...
            <div class="form-group" align="center">
                {!! Form::submit('Actualizar Asistencia' , ['class'=>'btn btn-primary btn-lg btn-block']) !!}
            </div>
            {!! Form::close() !!}
        </div>
    </div>
  ...
  <script type="text/javascript">
    $('#search').on('keyup',function(){
      ...
      $.ajax({
        ...
        data : {'search':$value, 'idinforme':{{$asistente['idinforme']}} },
    ...
    function posAsistentes(page, search){
      ...
      $.ajax({
        type : 'GET',
        url : url+'?page='+page,
        data : {'search' : search, 'idinforme':{{$asistente['idinforme']}} },
      }).done(function(data){
        $('#tbody').html(data);
      })
    }
